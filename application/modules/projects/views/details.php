							...
							<section class="slim-scroll" data-height="auto" data-disable-fade-out="true" data-distance="0" data-size="5px" data-color="#333333">
								...
								<section id="setting-nav" class="hidden-xs">
									<ul class="nav nav-pills nav-stacked no-radius">
										...
										<?php } 
										if ($role == '1' OR $role == '3' OR $this -> applib -> project_setting('show_team_members',$p->project_id)) { ?>
							...
							<header class="header bg-white b-b clearfix">
								<div class="row m-t-sm">
									<div class="col-sm-12 m-b-xs">
									 ...
									 <a href="<?=base_url()?>projects/view/<?=$p->project_id?>?group=<?=$group?>&action=edit" class="btn btn-info btn-sm pull-right"><i class="fa fa-edit"></i> <?=lang('edit_project')?></a>

									 <?php } ?>

									 <?php if ($role == '1'){ ?>
									 <a href="<?=base_url()?>projects/invoice/<?=$p->project_id?>" class="btn btn-dark btn-sm pull-right" data-toggle="ajaxModal"><i class="fa fa-money"></i> <?=lang('invoice_project')?></a>
									 <a href="<?=base_url()?>projects/copy_project/<?=$p->project_id?>" data-toggle="ajaxModal" class="btn btn-primary btn-sm pull-right"><i class="fa fa-copy"></i> <?=lang('clone_project')?></a>
									 <?php } ?>
									 <?php if ($role == '1' OR $this -> applib -> allowed_module('delete_projects',$username)){ ?>
									 <a href="<?=base_url()?>projects/delete/<?=$p->project_id?>?group=<?=$group?>" data-toggle="ajaxModal" class="btn btn-default btn-sm pull-right"><i class="fa fa-trash-o"></i> <?=lang('delete_project')?></a>
									 <?php } ?>
									 <?php if ($role != '2'){
										if($p->timer == 'On') { $label = 'danger'; } else{ $label = 'success'; } 
										if ($p->timer == 'On') { ?>
										<a href="<?=base_url()?>projects/tracking/off/<?=$p->project_id?>" class="btn btn-sm btn-<?=$label?> pull-right"> <i class="fa fa-clock-o text-white"></i> <?=lang('stop_timer')?></a>
										<?php }else{ ?>
										<a href="<?=base_url()?>projects/tracking/on/<?=$p->project_id?>" class="btn btn-sm btn-<?=$label?> pull-right"> <i class="fa fa-clock-o text-white"></i> <?=lang('start_timer')?></a>
										<?php } ?>
